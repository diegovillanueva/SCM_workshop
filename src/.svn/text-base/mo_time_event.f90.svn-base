...
MODULE mo_time_event
  ...
  PUBLIC  :: events_nonequal    ! log<-(I:event, I:event)
...
CONTAINS
  ...
  !>
  !! Check if two events are non-equal.
  !!
  !! For now, events are only equal if their defining properties are equal.
  !! These are equivalent to the original io_time_event settings.
  !!
  FUNCTION events_nonequal(a, b)
    TYPE(time_event), INTENT(in) :: a, b
    LOGICAL events_nonequal
