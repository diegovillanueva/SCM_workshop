...
MODULE mo_soil
  ...
  ! Modules used: 
  ! 
  USE mo_netCDF, ONLY: FILE_INFO, BELOWSUR, SOILLEV
  ...
  USE mo_update_surf_up, ONLY: update_surf_up
  ...
  TYPE soil_param_type
   ...
   REAL(dp), POINTER    :: HeatCapacity(:)        !! Vol. heat capacity of dry soil [J/(m^3*K)]
  ...
  TYPE soil_type
     ...
     REAL(dp), POINTER, DIMENSION(:)   ::   &  !! (nland)
       ...
       skin_reservoire_evap_acc,            &  !! Skin reservoir evaporation (accumulated) - diagnostic
     ...
     REAL(dp), POINTER, DIMENSION(:,:) ::   &  !! (nland, ntiles)
       ...
       org_temperature,                     &  !! Organic layer temperature
       c_org_temperature,                   &  !! Coefficient to compute organic layer temperature
       d_org_temperature,                   &  !!                 "
...
!----------------------------------------------------------------------------------------------------------
  TYPE soil_diag_type
    REAL(dp), POINTER, DIMENSION(:) ::   &
      ...
      glacier_runoff_acc,                &
      glacier_runoff_accw,               &
      !sz net_radiation and qair_acc added
      net_radiation,                     &  !! net radiation at surface [Wm-2]
      qair_acc,                          &  !! realised surface humidity (needed for offline model forcing) [-]
...
!
  ...
  SUBROUTINE soil_init_memory(g_nland, l_nland, ntiles, useDynveg, soil, isStandalone, &
    ...
    ! code numbers of this routine are 35-35, 39-40, 42, 44, 47, 49-50, 55, 57, 59-68, 72-74, 76, 78-83, 85-86, 91-101, 
    ...
    CALL add(IO_soil, 'snow_melt_inst',          soil%snow_melt,             longname='Snow Melt Flux', &
             units='kg m-2 s-1',           ldims=dim3p, gdims=dim3, dimnames=dim3n, code=1, laccu=.FALSE., &
    ...
    IF (ldiag_soil) THEN
      ...
      CALL add(IO_soil, 'snow_evaporation',      soil%snow_evap_acc, longname='Snow Evaporation (avg)',        &
           ...
           lpost=.FALSE., contnorest=.TRUE.)
      CALL add(IO_soil, 'skin_evaporation',      soil%skin_reservoire_evap_acc, longname='Skin Reservoir Evaporation (avg)', &
    ...
    IF (soil_options%withPermafrost) THEN
      ...
      CALL add(IO_soil, 'c_snow_temperature', soil%c_snow_temperature,               &
                                      ldims=dim5p, gdims=dim5, dimnames=dim5n, code=23, lpost=.FALSE., leveltype=BELOWSUR)
      CALL add(IO_soil, 'd_snow_temperature', soil%d_snow_temperature,               &
                                      ldims=dim5p, gdims=dim5, dimnames=dim5n, code=24, lpost=.FALSE., leveltype=BELOWSUR)
      CALL add(IO_soil, 'snow_temperature',   soil%snow_temperature,               &
                                      ldims=dim5p, gdims=dim5, dimnames=dim5n, code=25, lpost=.FALSE., leveltype=BELOWSUR)
      CALL add(IO_soil, 'c_org_temperature',  soil%c_org_temperature,                &
                                      ldims=dim3p, gdims=dim3, dimnames=dim3n, code=26, lpost=.FALSE.)
      CALL add(IO_soil, 'd_org_temperature',  soil%d_org_temperature,                &
                                      ldims=dim3p, gdims=dim3, dimnames=dim3n, code=27, lpost=.FALSE.)
      CALL add(IO_soil, 'org_temperature',    soil%org_temperature,                &
                                      ldims=dim3p, gdims=dim3, dimnames=dim3n, code=28, lpost=.FALSE.)
      CALL add(IO_soil, 'thaw_depth',         soil%thaw_depth,           &
               units='cm',            ldims=dim3p, gdims=dim3, dimnames=dim3n, code=29, lpost=.FALSE.)
    ...
    ELSE
      soil%ice_content => soil%d_soil_temperature ! Though not used, ice_content is passed through a subroutine interface also for
                                                  ! no permafrost and must thus be assigned to something
    ENDIF
    ...
    CALL add(IO_diag, 'snow_melt_inst',          soil_diag%snow_melt,             longname='Snow Melt',                        &
         units='kg/m^2s',                  ldims=dim1p, gdims=dim1, dimnames=dim1n, laccu=.FALSE., code=1, lpost=lpost_echam,  &
         lmiss=.TRUE., missval=missing_value)
    ...
    IF (ldiag_soil) THEN
      ...
      CALL add(IO_diag, 'snow_evaporation', soil_diag%snow_evap_acc, longname='Snow Evaporation (avg)', &
           units='kg/m^2s', ldims=dim1p, gdims=dim1, dimnames=dim1n, laccu=.TRUE.,  code=80,            &
           lpost=.TRUE., lmiss=.TRUE., missval=missing_value, bits=24)
      CALL add(IO_diag, 'skin_evaporation',soil_diag%skin_reservoire_evap_acc,     longname='Skin Reservoir Evaporation (avg)', &
           units='kg/m^2s', ldims=dim1p, gdims=dim1, dimnames=dim1n, laccu=.TRUE.,  code=81,                                    &
           lpost=.TRUE., lmiss=.TRUE., missval=missing_value, bits=24)
      ...
      CALL add(IO_diag, 'csat_transpiration',         soil_diag%csat_transpiration,  longname='csat transpiration coefficient', &   
           ...
           lmiss=.TRUE., missval=missing_value)
      IF (soil_options%withPermafrost) THEN
        CALL add(IO_diag, 'ice_content',      soil_diag%ice_content,      longname='Soil Ice content of layer i [m]',          &
        ...
        CALL add(IO_diag, 'heatcond',         soil_diag%heatcond,         longname='Soil heat conductivity of layer i [J/ms]', &
             ...
             lpost=.TRUE., lmiss=.TRUE., missval=missing_value, bits=24)
        CALL add(IO_diag, 'org_temperature',  soil_diag%org_temperature,  longname='Organic layer temperature [K]',            &
             units='K',      ldims=dim1p, gdims=dim1, dimnames=dim1n, laccu=.FALSE.,  code=95,  &
             lpost=.TRUE., lmiss=.TRUE., missval=missing_value, bits=24)
        CALL add(IO_diag, 'snow_layer',       soil_diag%snow_layer,       longname='Number of snow layers',                    &
        ...
        CALL add(IO_diag, 'snow_temperature', soil_diag%snow_temperature, longname='Snow Temperature',                         &
             units='K',      ldims=dim6p, gdims=dim6, dimnames=dim6n, laccu=.FALSE., code=99,   &
             lmiss=.TRUE.,               missval=missing_value) 
  ...
  SUBROUTINE init_soil(grid, domain, soil_param, soil, IO_file_name, isStandalone, isRestart, useAlbedo, &
    ...
    IF (soil_options%withPermafrost) THEN
       ...
       ALLOCATE(soil_param%HeatCapacity(domain%nland))
       IF (p_parallel_io) THEN
       ...
       IF (p_parallel_io) zreal2d_ptr => zzreal2d(:,:)
       CALL scatter_field(zreal2d_ptr, zreal2d)
       soil_param%HeatCapacity = PACK(zreal2d, MASK=domain%mask)
    ...
    IF (ldiag_soil) THEN
      ...
      soil_diag%skin_reservoire_evap_acc       = 0._dp
    ...
    IF (soil_options%withPermafrost) THEN
      ...
      soil_diag%org_temperature                  = 0._dp
      soil_diag%snow_layer                       = 0._dp
    ...
    !>>ZK: Prevent bounds-checking failure in soil temperature initialisation
    IF (domain%nland > 0) THEN
    !<<ZK
    ...
    IF (ldiag_soil) THEN
      ...
      soil%skin_reservoire_evap_acc       = 0._dp
    ...
    IF (soil_options%withPermafrost) THEN
      ...
      soil%heatcap           (:,:,:)      = 0._dp
      soil%heatcond          (:,:,:)      = 0._dp
      soil%c_org_temperature (:,:)        = 0._dp
      soil%d_org_temperature (:,:)        = 0._dp
      soil%org_temperature   (:,:)        = soil%surface_temperature(:,:)
      ...
      soil%snow_temperature  (:,:,:)      = soil%soil_temperature(:,:,:)
    ENDIF
  ...
  SUBROUTINE update_soil(nidx, surface, soil, soil_param, useDynveg, &
    ...
    USE mo_jsbach_constants, ONLY : Gravity, RhoH2O, SpecificHeatDryAirConstPressure, &
         ...
         GasConstantWaterVapor, zsigfac, zepsec, zqsncr, tmelt
...
! Local variables
    REAL(dp), DIMENSION(nidx) ::               &
         ...
         evapotranspiration_no_snow_skin_avg,  & ! Evapotranspiration without that from snow and the skin reservoir (box avg)
    ...
    REAL(dp), DIMENSION(nidx) ::                                  &
         ...
         snow_accum_avg, glacier_runoff_avg
...
!    INTEGER  :: jlpoint                              ! Absolute kpoints Index of water balance test grid box
    INTEGER  :: iMin                                  ! First layer to process (diff. with and without permafrost) 
    REAL(dp) :: skin_res_evap_tile(nidx,soil%ntiles)  ! Skin reservoir evaporation from update_surf_up per tile [m]
    REAL(dp) :: skin_reservoire_evap(nidx)            ! Total-Skin reservoir evaporation [m]
    ...
    REAL(dp) :: soil_water_pre(nidx)                  ! Soil water storages of previous time step
    REAL(dp) :: water_balance(nidx)                   ! Soil water balance within time step [mm]
    REAL(dp) :: dum(nidx)                             ! Array for various temporary data
    REAL(dp) :: zdum_layer(nidx, soil%nsoil)          ! Dummy array for multi-layers, e.g. field capacity
    REAL(dp) :: soil_ice_avg(nidx)                    ! Soil ice content averaged over tiles, summed over layers
!
...
!   Thus, tiling makes no sense and leads to errors in Back-calculations of bare soil evap.
    REAL(dp) :: SkinReservoire(nidx)                  ! Mean skin reservoir [m]
    REAL(dp) :: SkinReservoire_tile(nidx,soil%ntiles) ! Mean skin reservoir distributed on tiles [m]
    REAL(dp) :: SnowDepthCan(nidx)                    ! Mean snow on canopy [m]
...
!          leading to errors in the water balance, especially for the multi-layer hydrology scheme.
    ...
    IF ((nsoil > 1) .OR. (ldiag_soil)) THEN
      CALL average_tiles(soil%skin_reservoir(kidx0:kidx1,:),                              &
           surface%is_present(kidx0:kidx1,:) .AND. .NOT. surface%is_lake(kidx0:kidx1,:) , &
           surface%cover_fract(kidx0:kidx1,:), SkinReservoire(1:nidx) )
!
!   *** Save sum of water storages from previous time step
      CALL average_tiles(canopy_snow(1:nidx, :),                                          &
           surface%is_present(kidx0:kidx1,:) .AND. .NOT. surface%is_lake(kidx0:kidx1,:) , &
           surface%cover_fract(kidx0:kidx1,:), SnowDepthCan(1:nidx) )
      IF (nsoil > 1) THEN
        DO i=kidx0, kidx1
          soil_water_pre(i - kidx0 + 1) = SUM( soil%layer_moisture(i,1:soil%nsoil) )
        ENDDO
        DO itile=1,ntiles
      ...
      ELSE
        CALL average_tiles(soil%moisture(kidx0:kidx1,:), surface%is_present(kidx0:kidx1,:) &
             .AND. .NOT. surface%is_lake(kidx0:kidx1,:), surface%cover_fract(kidx0:kidx1,:), &
             soil_water_pre(1:nidx)  )
      ENDIF
      soil_water_pre(1:nidx) = soil_water_pre(1:nidx) + SkinReservoire(1:nidx) + SnowDepthCan(1:nidx) + snow_avg(1:nidx)
      IF (soil_options%withPermafrost) then
        soil_ice_avg(1:nidx)=0._dp
        DO i=1, soil%nsoil
          CALL average_tiles(soil%ice_content(kidx0:kidx1,i,:), surface%is_present(kidx0:kidx1,:) &
               .AND. .NOT. surface%is_lake(kidx0:kidx1,:), surface%cover_fract(kidx0:kidx1,:), &
               zdum_layer(1:nidx, i) )
          soil_ice_avg(1:nidx) = soil_ice_avg(1:nidx) + zdum_layer(1:nidx, i)
        END DO
        soil_water_pre(1:nidx) = soil_water_pre(1:nidx) + soil_ice_avg(1:nidx)
      ENDIF
    ENDIF
    ...
    DO itile=1,ntiles
       ...
       IF (nsoil > 1) SkinReservoire(1:nidx) = soil%skin_reservoir(kidx0:kidx1,itile)

       CALL update_surf_down( nidx,                                                  &
            air_moisture(1:nidx),                                                    &
            soil%surface_temperature_unfiltered(kidx0:kidx1,itile), wind10(1:nidx),  &
            air_temperature(1:nidx), soil%skin_reservoir(kidx0:kidx1,itile),         &
            ...
            evapotranspiration_no_snow_skin(1:nidx,itile),                           &
            rain, snow,                                                              &
            surface%is_present(kidx0:kidx1,itile),                                   &
            surface%is_glacier(kidx0:kidx1,itile),                                   &
            glacier_precip_minus_evap(:,itile),                                      &
            snow_accum(1:nidx,itile),                                                &
            snow_melt(1:nidx,itile), glacier_melt(1:nidx,itile),                     &
            glacier_runoff(1:nidx,itile), melt_water_excess(1:nidx,itile),           &
            tte_corr(1:nidx,itile))

!
!      update_surf_up is called for each tile, multi-layer hydrology routine is without tiles
...
!                   afterwards using new temporary field zlayer_moisture_tile
       IF (nsoil > 1) THEN
         ...
         SkinReservoire_tile(1:nidx,    itile) = SkinReservoire_tile(1:nidx, itile) + &
                                                 soil%skin_reservoir(kidx0:kidx1,itile) - SkinReservoire(1:nidx)

         IF (soil_options%withPermafrost) THEN
             CALL update_soiltemp_5lyr(nidx, soil%ntsoil, soil%cdel, soil%cmid        &
                             ...
                             , soil%c_org_temperature(kidx0:kidx1,itile)              &
                             , soil%d_org_temperature(kidx0:kidx1,itile)              &
                             , soil%org_temperature(kidx0:kidx1,itile)                &
                             , soil%ice_content(kidx0:kidx1,:,itile)                  &
                             , soil_param%HeatCapacity(kidx0:kidx1)                   &
                             , soil_param%HeatConductivity(kidx0:kidx1)               &
                             , soil%heatcap(kidx0:kidx1,:,itile)                      &
         ...
         ELSE
           CALL update_soiltemp(nidx,soil%ntsoil, soil%cdel(:), soil%cmid(:)                                        &
         ...
         CALL update_surf_up(nidx, nsoil, soil_options%withPermafrost,                                    &
            SkinReservoire_tile(1:nidx, itile), WetSkin_frac(1:nidx), MaxSkinResCap(1:nidx),              &
            soil%moisture(kidx0:kidx1,itile), soil_param%MaxMoisture(kidx0:kidx1),                        &
            ...
            surface%is_present(kidx0:kidx1,itile),                                                        &
            surface%is_glacier(kidx0:kidx1,itile),                                                        &
            surface_runoff(1:nidx,itile), drainage(1:nidx,itile), melt_water_excess(1:nidx,itile),        &
    ...
    ! soil%snow_melt is the combined snow/ice melt on land and glacier points.
    ...
    IF ((nsoil > 1) .OR. (ldiag_soil)) THEN
      CALL average_tiles(skin_res_evap_tile(1:nidx,:), surface%is_present(kidx0:kidx1,:)                &
                        .AND. .NOT. surface%is_lake(kidx0:kidx1,:), surface%cover_fract(kidx0:kidx1,:), &
                        skin_reservoire_evap)
      CALL average_tiles(evapotranspiration_no_snow_skin(1:nidx,:), surface%is_present(kidx0:kidx1,:)   &
                        .AND. .NOT. surface%is_lake(kidx0:kidx1,:), surface%cover_fract(kidx0:kidx1,:), &
                        evapotranspiration_no_snow_skin_avg)
    ...
    IF (nsoil > 1) THEN
      ...
      CALL average_tiles(SkinReservoire_tile(1:nidx, :),                                  &
           surface%is_present(kidx0:kidx1,:) .AND. .NOT. surface%is_lake(kidx0:kidx1,:) , &
           surface%cover_fract(kidx0:kidx1,:), SkinReservoire(1:nidx) )
      DO itile=1,ntiles
        DO i=1, nidx
          ...
          IF (MaxSkinResCap(i) > EPSILON(1._dp)) THEN
            soil%skin_reservoir(j, itile) = MIN(SkinReservoire(i)/MaxSkinResCap(i), 1._dp) * skin_reservoir_max(i, itile)
          ELSE    
    ...
    DO itile=1,ntiles
      ...
      IF (.NOT. soil_options%withPermafrost) &
        soil%soil_temperature(kidx0:kidx1,1,itile) = surface_temperature_unfilt_avg(:)
    END DO
    iMin = 2
    IF (soil_options%withPermafrost) iMin = 1 ! With permafrost,topmost soil layer is not in contact with atm.due to organic layer
    DO isoil=iMin,soil%ntsoil
...
!          evapotranspiration_no_snow_skin_avg(1:nidx),    &  ! [m]
...
!   endif
    ...
    IF (ldiag_soil) THEN
      IF (nsoil > 1) THEN
!
!    *** Calculate Soil water balance for each grid box. While the upper routine may be used
!    *** for extensive, this lower routine just gives out possible errors in the water balance.
        IF (soil_options%withPermafrost) then
           soil_ice_avg(1:nidx)=0._dp
           DO i=1, soil%nsoil
             CALL average_tiles(soil%ice_content(kidx0:kidx1,i,:), surface%is_present(kidx0:kidx1,:) &
             ...
             soil_ice_avg(1:nidx) = soil_ice_avg(1:nidx) + zdum_layer(1:nidx, i)
           END DO
           dum(1:nidx) = soil_moisture_avg(1:nidx) + soil_ice_avg(1:nidx)
           zdum_layer(1:nidx, :) = soil%layer_moisture(kidx0:kidx1, :) + zdum_layer(1:nidx, :)
        ELSE
           dum(1:nidx) = soil_moisture_avg(1:nidx)
           zdum_layer(1:nidx, :) = soil%layer_moisture(kidx0:kidx1, :)
        ENDIF
      ...
      soil%water_balance(kidx0:kidx1) = soil%water_balance(kidx0:kidx1) + water_balance(1:nidx)
      ...
      soil%bare_soil_evap_acc(kidx0:kidx1) = soil%bare_soil_evap_acc(kidx0:kidx1)   &    
          + evapotranspiration_no_snow_skin_avg(1:nidx) * RhoH2O - dum(1:nidx) * delta_time
      soil%snow_evap_acc(kidx0:kidx1) = soil%snow_evap_acc(kidx0:kidx1) + snow_evap(1:nidx) * delta_time
      soil%skin_reservoire_evap_acc(kidx0:kidx1) = soil%skin_reservoire_evap_acc(kidx0:kidx1) + &
                                                   skin_reservoire_evap(1:nidx) * RhoH2O
...
!   *** Calculate changes for separate bare soil moisture storage
    CALL average_tiles(soil%transpiration(kidx0:kidx1, :), surface%is_present(kidx0:kidx1,:) &
        ...
        dum(1:nidx) )
!
!   *** [evapotranspiration_no_snow_skin_avg] = m, [soil%transpiration] = [dum] = kg/m^2/s = mm/s
    bare_soil_evap(1:nidx) = evapotranspiration_no_snow_skin_avg(1:nidx) - dum(1:nidx) * delta_time / RhoH2O
...
!!!         soil_param%PotMoisture(kidx0:kidx1)   )
  ...
  SUBROUTINE soil_diagnostics(surface, soil)
    ...
    IF (soil_options%withPermafrost) THEN
       ...
       END DO
       CALL average_tiles(soil%org_temperature(kidx0:kidx1,1:ntiles), mask, fract, &
               soil_diag%org_temperature(kidx0:kidx1))
       CALL average_tiles(soil%nsnow_layer(kidx0:kidx1,1:ntiles), mask, fract, &
               soil_diag%snow_layer(kidx0:kidx1))
    ...
    IF (ldiag_soil) THEN
      ...
      ENDIF
      soil_diag%water_balance(kidx0:kidx1)            = soil%water_balance(kidx0:kidx1)
      ...
      soil_diag%skin_reservoire_evap_acc(kidx0:kidx1) = soil%skin_reservoire_evap_acc(kidx0:kidx1) 
