...
MODULE mo_mpi
  ...
  USE mpi
#ifdef __prism
  USE mo_couple_wrap,  ONLY: oasis_ok, oasis_init_comp, oasis_get_localcomm, &
                             oasis_abort
#ifdef __oa3mct
  USE mo_couple_wrap,  ONLY: oasis_set_couplcomm
#endif
#endif /* __prism */
#endif
  ...
  PUBLIC :: p_set_communicator, p_init_communicators, p_init_model_communicator
  PUBLIC :: p_is_iope  ! used in mo_io_server
  PUBLIC :: p_probe
  PUBLIC :: p_inc_request
#ifndef NOMPI
  PUBLIC :: p_request, p_irequest
#endif
  PUBLIC :: p_bcast_int_1d    ! to allow passing to subroutine 
...
!            MPI_INFO_NULL, MPI_ADDRESS_KIND
  ...
  PUBLIC :: p_info_null, p_comm_null

#ifdef __prism
  INTEGER, PUBLIC :: prism_model_number
  CHARACTER(len=132), PUBLIC :: prism_model_name
#endif
...
!!$#endif
  ...
  INTEGER :: p_comm_null = MPI_COMM_NULL
#else
  INTEGER :: p_info_null = 0
  INTEGER :: p_comm_null = 0
#endif            
  ! non blocking calls
...
!    CHARACTER   :: yyg = 'A'
    ...
    ! Executable statements:
     ...
     WRITE (nerr, '(a/t8,a)') 'ERROR: In coupled mode specification of either '// &
       &'-Duse_comm_MPI1 or -Duse_comm_MPI2 is required. ', &
        'p_start: aborting run, please recompile.'
     CALL p_abort
#endif
#if (defined (__oa3mct) && defined (use_comm_MPI2))
     WRITE (nerr, '(a/t8,a/t8,a)') 'ERROR: OASIS3-MCT does not support dynamically '// &
       &'spawned processes (MPI-2).', '-Duse_comm_MPI1 instead of -Duse_comm_MPI2 '// &
       &'is required. ', 'p_start: aborting run, please recompile.'
     CALL p_abort
#endif
#endif
#endif
    ...
    CALL MPI_ERRHANDLER_SET(MPI_COMM_WORLD, MPI_ERRORS_RETURN, p_error)
    IF (p_error /= MPI_SUCCESS) THEN
    ...
    CALL oasis_init_comp (prism_model_number, TRIM(prism_model_name), &
         p_error)
    IF (p_error /= oasis_ok) &
      CALL oasis_abort(0, 'p_start', &
          'oasis_init_comp failed for model '//TRIM(prism_model_name))

    CALL oasis_get_localcomm(p_global_comm, p_error)
    IF (p_error /= oasis_ok) &
      CALL oasis_abort(0, 'p_start', &
          'oasis_get_localcomm failed for model '//TRIM(prism_model_name))
#else
    ...
    IF (mype == 0) THEN    
      ...
      ELSE
        ...
        WRITE (nerr,'(1x,a,a,a,/,1x,a,a,a)') &
             ...
             ' or not available to ', TRIM(yname), ' root PE. Set to 1!'
        env_threads = 1
      ENDIF
...
!!$       CALL MPI_SIZEOF(yyg, p_char_byte, p_error)
  ...
  SUBROUTINE p_update_model_communicator(model_comm)
...
#ifndef NOMPI
    CALL MPI_COMM_RANK (model_comm, mype, p_error)
    IF (p_error /= MPI_SUCCESS) THEN
       WRITE (nerr,'(a)') ' MPI_COMM_RANK failed.'
       WRITE (nerr,'(a,i4)') ' Error =  ', p_error
       CALL p_abort
...
#ifndef NOMPI
    CALL MPI_COMM_SIZE (model_comm, npes, p_error)
    IF (p_error /= MPI_SUCCESS) THEN
       WRITE (nerr,'(a,i4,a)') ' PE: ', mype, ' MPI_COMM_SIZE failed.'
       WRITE (nerr,'(a,i4)') ' Error =  ', p_error
       CALL p_abort
    ...
    ! update relevant module information

    IF (npes < 2) THEN
      p_parallel = .FALSE.
      p_parallel_io = .TRUE.   ! can always do I/O
      IF (mype == 0) THEN
        WRITE (nerr,'(a)') '  Update: single processor run.'
      END IF
      p_pe = 0
      p_nprocs = 1
    ELSE
      ...
      END IF
      IF (mype == 0) THEN
        WRITE (nerr,'(a,i0,a)') '  Update: run on ', npes, ' processors.'
      END IF
      p_pe = mype
      p_nprocs = npes
    END IF

  END SUBROUTINE p_update_model_communicator

  ! Create communicator for model compute PEs without io-servers
  SUBROUTINE p_init_model_communicator(p_global_comm, nproca, nprocb, nprocio, &
                                       iomode, PartInFlate)

    INTEGER,  INTENT(IN)  :: p_global_comm  ! Global MPI communicator (all PEs)
    INTEGER,  INTENT(IN)  :: nproca         ! #of PEs for latitude dimension
    INTEGER,  INTENT(IN)  :: nprocb         ! #of PEs for longitude dimension
    INTEGER,  INTENT(IN)  :: nprocio        ! #of I/O servers
    INTEGER,  INTENT(IN)  :: iomode         ! IO mode
    REAL(sp), INTENT(IN)  :: PartInFlate    ! Inflation factor for maximum size of 
                                            ! local arrays
    INTEGER :: pio_namespace ! CDI-PIO namespace

#if ! (defined (HAVE_YAXT) && defined (HAVE_CDIPIO))
    IF (nprocio > 0) THEN
      WRITE (nerr, '(a/t8,a/t8,a)') 'ERROR: For parallel or asynchronous output '// &
         &'specification of -DHAVE_YAXT and -DHAVE_CDIPIO is required.', &
          'Please, recompile and link with YAXT and CDI-PIO libary OR '// &
         &'set nprocio=0 in namelist PARCTL.', &
          'p_init_model_communicator: aborting run.'
      CALL p_abort
    ENDIF
#endif
    ...
    IF (nprocio < 0) THEN
        WRITE (nerr,'(a)') &
             'ERROR: Number of I/O servers nprocio is negative.' 
        CALL p_abort
    ENDIF
    
    IF (nprocio > nproca*nprocb) THEN
        WRITE (nerr,'(a)') &
...
#else
      WRITE (nerr,'(a)') 'coupler oasis3 unhandled yet, use nprocio=0'
      CALL p_abort
#endif

#else  /* uncoupled */
      p_all_comm = pioInit(p_global_comm, nprocio, iomode, pio_namespace, &
                           partInFlate, cdiPioNoPostCommSetup)
      IF (p_all_comm == p_comm_null) RETURN
#endif

      CALL NamespaceSetActive(pio_namespace)
      !CALL pioNamespaceSetActive(pio_namespace)   ! for cdi <1.6.3
      CALL p_update_model_communicator(p_all_comm)

#ifdef __prism  /* coupled */
#ifdef __oa3mct
      CALL oasis_set_couplcomm(p_all_comm, p_error)
      IF (p_error /= oasis_ok) &
         CALL oasis_abort(0, 'p_init_model_communicator', &
             'oasis_set_couplcomm failed for echam6')
#else
      CALL oasis_abort(0, 'p_init_model_communicator', &
  ...
  END SUBROUTINE p_init_model_communicator
