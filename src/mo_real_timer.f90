...
MODULE mo_real_timer
  ...
  USE mo_util_db_timings, ONLY: open_db_timings, close_db_timings,   &
                                insert_db_experiment, insert_db_job, &
                                insert_db_timing, connection_info
#ifdef _OPENMP
...
!                                omp_get_dynamic, omp_set_dynamic
  ...
  TYPE rtmax_type                    !++mgs: additional timer structure for reporting
     REAL(dp) :: tot
     REAL(dp) :: min
     REAL(dp) :: max
     REAL(dp) :: avg
  ...
  TYPE(rtmax_type) :: reportt     !++mgs: collect max values of all timers to decide on report format
                                  ! (full report only)
!$OMP THREADPRIVATE(rt)

#if defined (__SX__) || defined (ES)
! necessary due to a bug in NECs OMP implementation
LOCAL COMMON /th_real_timer/ rt, reportt
#else
#ifdef _OPENMP
  COMMON /th_real_timer/ rt, reportt
    ...
    IF (PRESENT(short)) THEN
      IF (short) THEN
        IF (it == -1 .AND. p_parallel_io) THEN
          iret = open_db_timings(connection_info)
          IF (iret == 1) THEN
            CALL finish('timer_report','Could not connect to DB!')
          ENDIF
          iret = insert_db_experiment()
          iret = insert_db_job()
        ENDIF
        CALL timer_report_short(it)
        IF (it == -1 .AND. p_parallel_io) THEN
          CALL close_db_timings()
        ENDIF
        RETURN
      ENDIF
    ENDIF
...
!$OMP END PARALLEL
    ...
    IF (p_parallel_io) THEN
      ...
      DO iit = sel_num, 1, -1
        ...
        WRITE (e_str,'(f6.2)') 100*e
        WRITE (message_text,'(a,4a10,1x,a6)') ' '//srt(it)%text(1:27)//' :  ', &
             min_str, avg_str, max_str, sum_str, e_str
        CALL message ('',message_text,all_print=.TRUE.)
        iret = insert_db_timing(srt(it)%text(1:27), &
             res(i_min,isel), avg, res(i_max,isel), res(i_sum,isel), e)
      ENDDO
      IF (decorate) CALL message ('',separator,all_print=.TRUE.)
  ...
  SUBROUTINE timer_report_full(itimer)
    ...
    IF (p_nprocs > 1) THEN
      WRITE (message_text,'(a4,i5,a)') &
           'PE: ', p_pe, '                  calls      t_min   t_average       t_max     t_total '
      CALL message ('',message_text,all_print=.TRUE.,adjust_right=.TRUE.)
    ELSE
    ...
    ELSE
!++mgs: obtain maximum values for min, avg, max and total times
      icnt = 0
      reportt%tot = 0._dp
      reportt%min = 0._dp
      reportt%max = 0._dp
      reportt%avg = 0._dp
      DO it = 2, top_timer       ! exclude first time (=total)
        IF (rt(it)%stat /= rt_undef_stat) THEN
          icnt = icnt + 1
          IF (rt(it)%tot > reportt%tot) reportt%tot = rt(it)%tot
          IF (rt(it)%min > reportt%min) reportt%min = rt(it)%min
          IF (rt(it)%max > reportt%max) reportt%max = rt(it)%max
          zavg = rt(it)%tot/MAX(1,rt(it)%call_n)
          IF (zavg > reportt%avg) reportt%avg = zavg
        END IF
      END DO
!--mgs
      DO it = 1, top_timer
...
!$OMP END PARALLEL
  ...
  SUBROUTINE report(it)
    ...
    IF ( rt(it)%call_n > 0 ) THEN      
      min_str = time_str(rt(it)%min, reportt%min)
      avg_str = time_str(avg,        reportt%avg)
      max_str = time_str(rt(it)%max, reportt%max)
    ELSE
      RETURN
      !min_str=''
      !avg_str=''
      !max_str=''
    ENDIF
    tot_str = time_str(total,   reportt%tot)
    IF (tid == -1) THEN
      WRITE (message_text,'(a24,i6,4a12)') '    '//srt(it)%text, &
           rt(it)%call_n, min_str, avg_str, max_str, tot_str
      CALL message ('',message_text,all_print=.TRUE.,adjust_right=.TRUE.)
    ELSE
      WRITE (message_text,'(i2,a22,i6,4a12)') tid,': '//srt(it)%text, &
           rt(it)%call_n, min_str, avg_str, max_str, tot_str
      CALL message ('',message_text,all_print=.TRUE.,adjust_right=.TRUE.)
    ENDIF
...
!!mgs!!  CHARACTER(len=12) FUNCTION time_str(ts)
!!mgs!!    REAL(dp), INTENT(in) :: ts
!!mgs!!    
!!mgs!!    REAL(dp) :: rest
!!mgs!!
!!mgs!!    INTEGER :: d, h, m, s
!!mgs!!
!!mgs!!    CHARACTER(len=2) :: d_str, h_str, m_str, s_str
!!mgs!!    CHARACTER(len=12) :: x
!!mgs!!    
!!mgs!!    rest = ts
!!mgs!!    
!!mgs!!    d = INT(rest/(3600*24))
!!mgs!!    rest = rest-d*(3600*24)
!!mgs!!    IF (d > 99) THEN
!!mgs!!      x = '>99d'
!!mgs!!      time_str = ADJUSTR(x)
!!mgs!!      RETURN
!!mgs!!    ENDIF
!!mgs!!    WRITE(d_str,'(i2.2)') d
!!mgs!!    
!!mgs!!    h = INT(rest/3600)
!!mgs!!    rest = rest-h*3600
!!mgs!!    WRITE(h_str,'(i2.2)') h
!!mgs!!    
!!mgs!!    m = INT(rest/60)
!!mgs!!    rest = rest-m*60
!!mgs!!    WRITE(m_str,'(i2.2)') m
!!mgs!!    
!!mgs!!    s = INT(rest)
!!mgs!!    rest = rest-s
!!mgs!!    WRITE(s_str,'(i2.2)') s
!!mgs!!    
!!mgs!!    IF (d > 0) THEN        
!!mgs!!      x = TRIM(d_str)//'d'//TRIM(h_str)//'h'
!!mgs!!    ELSEIF (h > 0) THEN
!!mgs!!      x = TRIM(h_str)//'h'//TRIM(m_str)//'m'
!!mgs!!    ELSEIF (m > 0) THEN
!!mgs!!      x = TRIM(m_str)//'m'//TRIM(s_str)//'s'
!!mgs!!    ELSEIF (ts >= 1.0_dp) THEN
!!mgs!!      WRITE(x,'(f6.3,a)') ts, 's'
!!mgs!!    ELSE
!!mgs!!      WRITE(x,'(f6.5,a)') ts, 's'
!!mgs!!    ENDIF
!!mgs!!    time_str = ADJUSTR(x)
!!mgs!!    
!!mgs!!  END FUNCTION time_str

  CHARACTER(len=12) FUNCTION time_str(ts, tmax)
    REAL(dp), INTENT(in) :: ts, tmax
    ...
    ! decide on reporting unit based on tmax:
    ! tmax < 10 min --> seconds
    ! tmax <  3 h   --> minutes
    ! tmax <  3 d   --> hours
    ! tmax >= 3 d   --> days
    ! (tmax is given in seconds)

    IF (tmax >= 3._dp*24._dp*3600._dp) THEN
      WRITE(x,'(i2.2,a1,i2.2,a1,i2.2,a1,i2.2,a1)') d, 'd', h, 'h', m, 'm', s, 's'
    ELSE IF (tmax >= 3._dp*3600._dp) THEN
      h = INT(ts/3600)
      WRITE(x,'(i3,a1,i2.2,a1,i2.2,a1)') h, 'h', m, 'm', s, 's'
    ELSE IF (tmax >= 600._dp) THEN
      m = INT(ts/600)
      WRITE(x,'(i3,a1,i2.2,a1)') m, 'm', s, 's'
