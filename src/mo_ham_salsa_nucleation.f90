...
!>
!! \filename 
!! mo_ham_salsa_nucleation.f90
!!
!! \brief
!! Contains subroutines and functions that are used to new particle formation by nucleation
!!
...
!!
!! \details
!! This module contains the nucleation routine and subroutines for binnucl, ternucl, kinnucl
!! and actnucl. Nucleation routine is called from mo_ham_salsa_dynamics condensation subroutine. 
...
CONTAINS
  ...
  SUBROUTINE nucleation(kproma, kbdim,  klev,   krow,                     &
    ...
    ! >> thk: VBS
    pcsa(1:kproma,:)   = pc_gas(1:kproma,:,isubm_so4g)
    pcocnv(1:kproma,:) = pc_gas(1:kproma,:,isubm_ocnv)
    ! << thk
    ...
    CASE(1) ! binary H2SO4-H2O nucleation
       ...
       zc_h2so4(1:kproma,:) = pcsa(1:kproma,:)*1.e-6_dp           ! [#/cm3]

       ppbl_bin(1:kproma) = klev
    ...
    CASE(4) ! ternary H2SO4-H2O-NH3 nucleation
       ...
       CALL finish('mo_ham_salsa_nucleation','Specify NH3 mixing ratio')

       zmixnh3(1:kproma,:) = 0._dp
    ...
    CASE(5) ! organic nucleation, J~[ORG] or J~[ORG]**2
       ...
       zc_org(1:kproma,:) = pcocnv(1:kproma,:)*1.e-6_dp           ! [#/cm3]
       zc_h2so4(1:kproma,:) = pcsa(1:kproma,:)*1.e-6_dp           ! [#/cm3]
    ...
    CASE(6) ! sum of h2so4 and organic activation type nucleation
       ...
       zc_h2so4(1:kproma,:) = pcsa(1:kproma,:)*1.e-6_dp
    ...
    CASE(7) ! heteromolecular nucleation (J~[H2SO4]*[ORG])
       ...
       zc_h2so4(1:kproma,:) = pcsa(1:kproma,:)*1.e-6_dp
       zc_org(1:kproma,:) = pcocnv(1:kproma,:)*1.e-6_dp
    ...
    zcsa_local(1:kproma,:) = pcsa(1:kproma,:)
    zcocnv_local(1:kproma,:) = pcocnv(1:kproma,:)
...
!          IF (pcsa(ii,jj)+pcocnv(ii,jj) < 1._dp) CYCLE
          ...
          zmfp   = 3._dp*zdfvap*sqrt(pi*(speclist(id_so4)%moleweight/1000.)/(8._dp*argas*ptemp(ii,jj)))       ! mean free path [m]
          ...
          IF (nj3 == 1) THEN       ! Parametrization of J3 by Kerminen and Kulmala (2002)
             ...
             ELSE
                ...
                zgamma = 0.23_dp*(zdcrit(ii,jj)*1.e9_dp)**0.2_dp &! fxm: can we use simple version of zgamma given in Kerminen et al.?
                     ...
                     *(ptemp(ii,jj)/293._dp)**(-0.75_dp)*(speclist(id_so4)%density/1000._dp)**(-0.33_dp)
                ! [nm2*m2/h] (22)
          ...
          ELSEIF (nj3 > 1) THEN
             ...
             zm_c = 4._dp/3._dp*pi*(zdcrit(ii,jj)/2._dp)**3*speclist(id_so4)%density!(speclist(id_so4)%density*pxsa+speclist(id_oc)%density*pxocnv)    ! [kg] T�ss� oletettu hiukkasen massaksi vain H2SO4
             zm_x = 4._dp/3._dp*pi*(reglim(1)/2._dp)**3*speclist(id_so4)%density!(speclist(id_so4)%density*pxsa+speclist(id_oc)%density*pxocnv)       ! [kg]
             zm_2 = 4._dp/3._dp*pi*(pdwet(ii,jj,:)/2._dp)**3*speclist(id_so4)%density!(speclist(id_so4)%density*pxsa+speclist(id_oc)%density*pxocnv)   ! [kg] 
             
             zcv_c = SQRT(8._dp*ak*ptemp(ii,jj)/(pi*zm_c))     ! [m/s]
             zcv_x = SQRT(8._dp*ak*ptemp(ii,jj)/(pi*zm_x))     ! [m/s]
             zcv_2 = SQRT(8._dp*ak*ptemp(ii,jj)/(pi*zm_2))     ! [m/s]
             ...
             zDc_c = ak*ptemp(ii,jj)*zCc_c/(3._dp*pi*zmyy*zdcrit(ii,jj))  ! D1, Diffusion coefficient of zdcrit-particle, [m2/s]
             zDc_x = ak*ptemp(ii,jj)*zCc_x/(3._dp*pi*zmyy*reglim(1))     ! D1, Diffusion coefficient of dx-particle
             zDc_2 = ak*ptemp(ii,jj)*zCc_2/(3_dp*pi*zmyy*pdwet(ii,jj,:))          
             ...
             ELSEIF (nj3 == 3) THEN
                ...
                ! GRscg = 1.5708e-6_dp*zlambda*zdcrit(ii,jj)**3*(zNnuc*1.e-6_dp)*zcv_c*avo*1.e-9_dp/3600._dp, [m/s]
                ...
                DO iteration = 1,5
                   ...
                   zGRtot = zGRclust*1.e-9_dp/(3600._dp) + &
                        1.5708e-6_dp*zlambda*zdcrit(ii,jj)**3*(zNnuc*1.e-6_dp)*zcv_c*avo*1.e-9_dp/3600._dp 
                   ! = GRcond + GRscg, m/s
                ...
                zGRtot = zGRclust*1.e-9_dp/(3600) + &
                     1.5708e-6_dp*zlambda*zdcrit(ii,jj)**3*(zNnuc*1.e-6_dp)*zcv_c*avo*1.e-9_dp/3600._dp ![m/s]

                zj3 = zjnuc(ii,jj)*exp(min(0._dp,-zgamma*zdcrit(ii,jj)*zCoagStot/zGRtot))  !(5a) [#/m3s]
                
             ELSE
                CALL finish('mod_aero_nucleation','Error in chosen nj3')
             END IF
...
!          pcocnv(ii,jj) = pcocnv(ii,jj) + zdelta_vap*pxocnv
  ...
  SUBROUTINE binnucl(kproma,    kbdim,      klev,                  &
    ...
    pnuc_rate(1:kproma,:) = 0._dp
    pd_crit(1:kproma,:)    = 1.e-9_dp

    zpbl(1:kproma) = ppbl(1:kproma)
    if (lnuctropo) then
          ...
          DO jj = 1,zpbl(ii) !  vertical grid
             ...
             IF (pn_crit_sa(ii,jj) < 4._dp) THEN
                ...
                zcoll =   zpcsa*zpcsa* &
                     ...
                     SQRT(6._dp*argas*zt/zm1+6._dp*argas*zt/zm2) * &
  ...
  !  coagcoeff = 8*sqrt(3*ak*ptemp*r_abs/dens_abs)
  ...
  SUBROUTINE kinnucl(kproma,     kbdim,        klev,          &
     ...
     pd_crit(1:kproma,:) = 7.9375e-10_dp ! [m]
...
END MODULE mo_ham_salsa_nucleation
