...
CONTAINS
  ...
  SUBROUTINE Set_JulianDay(ky, km, kd, ksec, my_day, lperpetual_year)
    ...
    IF (.NOT. lperp) THEN
      ...
      IF (kd < 1 .OR. idmax < kd) THEN
        WRITE (message_text, '(i2.2,a1,i2.2,a)') &
             idmax, '/', kd, ' day in months invalid.'
        CALL message('',message_text)
        CALL finish ('mo_time_base:Set_JulianDay' ,'Fatal error!')
      ENDIF
    ENDIF
  ...
  SUBROUTINE Set_JulianCalendar(my_day, ky, km, kd, ksec)
    ...
    zday  = REAL(my_day%day,wp)
    zfrac = REAL(my_day%fraction,wp)

    za   = FLOOR(zday+zfrac+0.5_wp)

    ! The calculation of zb causes eventually problems with precision 
    ! zb = floor((za-1867216.25_wp)/36524.25_wp)
    ! 1.0/36524.25 = 0.273790700698850764E-04
    ! zb = floor((za-1867216.25_wp)*0.273790700698850764E-04_wp)
    ! 1867216.25_wp)*0.273790700698850764E-04_wp = 51.1226445443780502865715_wp
    zb = FLOOR(0.273790700698850764E-04_wp*za-51.1226445443780502865715_wp)

    ! 1.0/4.0 = 0.250000000000000000E+00
    ! zc = za+zb-floor(zb/4)+1525
    zc = za+zb-FLOOR(zb*0.25_wp)+1525

#ifdef IAU
    IF (za < 2299161.0_dp) zc = za+1524.0_dp
#endif

    ! 1.0/365.25 = 0.273785078713210130E-02
    ! zd = floor((zc-122.1_wp)/365.25_wp)
    zd = FLOOR((zc-122.1_wp)*0.273785078713210130E-02_wp)

    ze = FLOOR(365.25_wp*zd)

    ! 1.0/30.6001 = 0.326796317659092617E-01
    ! zf = floor((zc-ze)/30.6001_wp)
    zf = FLOOR((zc-ze)*0.326796317659092617E-01_wp)

    
    kd = INT(zc-ze- FLOOR(30.6001_dp*zf))
    ! 1.0/14.0 = 0.714285714285714286E-01
    ! km = INT(zf-1-12*FLOOR(zf/14))
    km = INT(zf-1-12*FLOOR(zf*0.714285714285714286E-01_wp))
    ! no changes here as this is a interger division
  ...
  INTEGER FUNCTION Get_JulianMonLen(ky, km)
    ...
    CASE(2)
      ...
      ! decision rules written conservative!
