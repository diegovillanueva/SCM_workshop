...
#if defined (__PGI)
  TYPE(pe_decomposed) :: default_decomposed
#else
...
CONTAINS
  ...
  LOGICAL ,INTENT(in) :: lfullm
    ...
    INTEGER :: insef, irestf, jb, jmloc, icolset, iave, im, inm, jn, i
  ...
  SUBROUTINE print_decomposition (dc)
    ...
    CALL message('', message_text)
    CALL print_field('', RESHAPE(dc%mapmesh,(/SIZE(dc%mapmesh)/)))
