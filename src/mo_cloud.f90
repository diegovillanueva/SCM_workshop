...
#ifndef __ICON__
  USE mo_submodel_interface,   ONLY : cloud_subm_2 !SF
...
!>>SF Initialization of some intent(out) variables over the whole array
  !   because they are passed back to stream elements in physc, which may cause nproma dependency
  prsfl(:) = 0._wp
  pssfl(:) = 0._wp
!<<SF
...
!
    ...
    IF (lanysubmodel) THEN
      CALL cloud_subm_2(kproma,     kbdim,      klev,       ktdia,                     &
        &               krow,                                                          &
