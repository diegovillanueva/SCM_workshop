...
MODULE mo_gaussgrid
  ...
  USE mo_kind,      ONLY: dp
  USE mo_control,   ONLY: ngl, nhgl, nlon, earth_angular_velocity
  ...
  REAL(dp), ALLOCATABLE :: gl_coriol(:)   ! coriolis parameter, 2*earth_angular_velocity*mu
  ...
  REAL(dp), ALLOCATABLE :: gl_racst(:)    ! 1/(earth_radius*(1-mu**2)
...
!lk CALL gauaw_old(zgmu,zgw,ngl)
    ...
    DO jgl = 1, nhgl
      ...
      gl_coriol(jgl)       =  2*earth_angular_velocity*zgmu(jgl)
      gl_coriol(ngl-jgl+1) = -2*earth_angular_velocity*zgmu(jgl)
      ...
      gl_racst(jgl)        = 1.0_dp/(earth_radius*zcst)
      gl_racst(ngl-jgl+1)  = 1.0_dp/(earth_radius*zcst)
    END DO


    DO jlon = 1, nlon               ! double size for rotated domains 
      zl = 2*pi*(jlon-1.0_dp)/nlon    ! on decomposed grid
      sinlon(jlon) = SIN(zl)
    ...
    DO jgl=1,nhgl      
      gridarea(jgl)       = gl_budw(jgl)*4*pi*earth_radius**2
      gridarea(ngl+1-jgl) = gridarea(jgl)
      philat  (jgl)       = 180._dp/pi*ASIN(zgmu(jgl))
...
!------------------------------------------------------------------------------
  SUBROUTINE gauaw_old (pa, pw, nlat)
    ...
    DO jgl = 1, ins2
       z = REAL(4*jgl-1,dp)*pi/REAL(4*nlat+2,dp)
