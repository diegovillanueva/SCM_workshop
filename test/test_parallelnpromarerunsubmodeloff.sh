...
echo ''
echo '-------------------------------------------------------------------'
echo "Now running test model rev. ${TEST_REVISION} - configuration 2, test case 4: submodel=.false., no rerun"
echo '-------------------------------------------------------------------'
#1.1) T31L39, nproma=17, n CPUs, lrerun=.false., lco2=.true., 12 time steps
${SCR_DIR}/test_echam6_run.sh ${SCR_DIR} test ${TEST_ODIR} 00004rev${TEST_REVISION} ${TEST_MODEL} 17 $NPROCA .false. 00 32 .true.
echo '-------------------------------------------------------------------'
